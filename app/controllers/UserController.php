...
class UserController extends SecureController
{
	...
	function view($rec_id = null, $value = null)
	{
		...
		if ($record) {
			$page_title = $this->view->page_title = "Detail User";
		} else {
			if ($db->getLastError()) {
				$this->set_page_error();
			} else {
	...
	function add($formdata = null)
	{
		if ($formdata) {
			...
			$request = $this->request;
			// Fillable fields
			$fields = $this->fields = array("username", "nama_pengguna", "user_role_id", "nip", "nis", "password", "email", "photo");
			...
			// Validasi NIP/NIS berdasarkan user_role_id
			if ($modeldata['user_role_id'] == '2') { // Guru
				if (empty($modeldata['nip'])) {
					$this->view->page_error[] = "NIP is required for guru.";
				} else {
					$db->where("nip", $modeldata['nip']);
					if (!$db->has('guru')) {
						$this->view->page_error[] = "NIP tidak ditemukan di tabel guru!";
					}
				}
			} elseif ($modeldata['user_role_id'] == '3') { // Siswa
				if (empty($modeldata['nis'])) {
					$this->view->page_error[] = "NIS is required for siswa.";
				} else {
					$db->where("nis_siswa", $modeldata['nis']);
					if (!$db->has('siswa')) {
						$this->view->page_error[] = "NIS tidak ditemukan di tabel siswa!";
					}
				}
			}
			...
			if ($this->validated()) {
				...
				if ($this->validated()) {
					...
					if ($rec_id) {
						// Update user_id in guru table if role is guru
						if ($modeldata['user_role_id'] == '2') { // Guru
							$db->where("nip", $modeldata['nip']);
							$db->update("guru", ["user_id" => $rec_id]);
						}
						// Update user_id in siswa table if role is siswa
						if ($modeldata['user_role_id'] == '3') { // Siswa
							$db->where("nis_siswa", $modeldata['nis']);
							$db->update("siswa", ["user_id" => $rec_id]);
						}
	...
	function edit($rec_id = null, $formdata = null)
	{
		...
		$tablename = $this->tablename;
		// Editable fields
		$fields = $this->fields = array("id_user", "username", "nama_pengguna", "nip", "nis", "password", "email", "photo");
		if ($formdata) {
			$postdata = $this->format_request_data($formdata);
			$cpassword = $postdata['confirm_password'];
			$password = $postdata['password'];
